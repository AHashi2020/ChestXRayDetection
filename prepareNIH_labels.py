...
# --------------------------------------------------
nih_root = "NIH"
train_output_csv = "train_labels.csv"
val_output_csv = "val_labels.csv"
...
def parse_finding_labels(finding_string, unexpected_labels_set):
    label_dict = build_empty_label_dict()
...
def summarize_dataframe(dataframe, name):
    ...
    label_frame = dataframe[label_names]
...
# --------------------------------------------------
def main():
    print("Preparing NIH multi-label CSVs...", flush=True)
    ...
    while row_index < len(metadata_df):
        ...
        label_dict = parse_finding_labels(finding_string, unexpected_labels)
        ...
        while label_index < len(label_names):
            current_label = label_names[label_index]
            record[current_label] = label_dict[current_label]
            label_index += 1
    ...
    print("", flush=True)
    print("Finished building multi-label table.", flush=True)
    ...
    label_index = 0
    while label_index < len(label_names):
        column_order.append(label_names[label_index])
        label_index += 1

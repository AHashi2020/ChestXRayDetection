...
TTA_LEARNING_RATE = 1e-6
MIN_ACCEPTED_SAMPLES = 2
MAX_ADAPT_SAMPLES = 2

REUSE_SAVED_THRESHOLD = False
...
def build_tta_index_groups(active_specs, nih_label_to_index):
    """
    Build the exact model-output groups used for overlap-based VinDr evaluation.
    Example:
    - Pleural_Effusion -> [Effusion index]
    - Nodule_Mass -> [Mass index, Nodule index]
    """
    tta_index_groups = []
    tta_group_names = []
...
def overlap_entropy_from_logits(logits: torch.Tensor, tta_index_groups):
    """
    Compute entropy only on the overlap label groups used for VinDr evaluation.
    For grouped labels like Nodule_Mass, use the max probability across the group.
    Returns per-sample entropy: shape [B]
    """
...
def estimate_overlap_entropy_threshold(
    ...
    """
    Estimate the entropy gate using NIH validation images,
    but only on the overlap label groups used in VinDr evaluation.
    """
    model.eval()
    all_entropies = []
...
def tta_step_multilabel_overlap(
    ...
    """
    Adapt only on the overlap-label entropy, not all 14 NIH outputs.
    Also cap the update to the lowest-entropy accepted samples.
    """
    logits = model(images)
...
def main():
    ...
    # ---------------------------------------------
    # Build VinDr dataset first so TTA matches eval labels
    # ---------------------------------------------
    ...
    # ---------------------------------------------
    # Recompute threshold using overlap-only entropy
    # ---------------------------------------------
    ...
    if REUSE_SAVED_THRESHOLD and os.path.exists(OUTPUT_THRESHOLD_JSON):
        try:
            ...
            if saved_quantile == ENTROPY_QUANTILE and saved_groups == tta_group_names:
                ...
                print("Saved threshold does not match current overlap setup. Recomputing.")
        except Exception:
    ...
    if entropy_threshold is None:
        ...
        print("Estimated overlap-only multi-label entropy threshold:", entropy_threshold)
    ...
    if "selected_count" in tta_log.columns:
        print("Mean selected count:", float(tta_log["selected_count"].mean()))
